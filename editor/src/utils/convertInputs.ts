...
export async function convertInputValue(
  ...
  if (typeof value === "string") {
    if (value.length > UPLOAD_SIZE_THRESHOLD) {
      console.log(`${LOG_PREFIX} "${key}": string ${formatSize(value.length)} exceeds threshold, uploading to S3`);
      try {
        const info = await uploadString(key, value);
        console.log(`${LOG_PREFIX} "${key}": uploaded → ${info.url}`);
        return { type: "url", value: info.url };
      } catch (e) {
        console.warn(`${LOG_PREFIX} "${key}": upload failed, falling back to inline utf8:`, e);
      }
    } else {
      console.log(`${LOG_PREFIX} "${key}": string ${formatSize(value.length)} → inline utf8`);
    }
  ...
  if (typeof value === "object" && value instanceof Blob) {
    ...
    if (estimatedBase64Size > UPLOAD_SIZE_THRESHOLD) {
      console.log(`${LOG_PREFIX} "${key}": blob ${formatSize(value.size)} (type=${value.type || "unknown"}) exceeds threshold, uploading to S3`);
      try {
        const info = await uploadBlob(key, value);
        console.log(`${LOG_PREFIX} "${key}": uploaded → ${info.url}`);
        return { type: "url", value: info.url };
      } catch (e) {
        console.warn(`${LOG_PREFIX} "${key}": upload failed, falling back to inline base64:`, e);
      }
    } else {
      console.log(`${LOG_PREFIX} "${key}": blob ${formatSize(value.size)} → inline base64`);
    }
  ...
  if (typeof value === "object") {
    ...
    if (jsonString.length > UPLOAD_SIZE_THRESHOLD) {
      console.log(`${LOG_PREFIX} "${key}": json ${formatSize(jsonString.length)} exceeds threshold, uploading to S3`);
      try {
        const info = await uploadJson(key, value);
        console.log(`${LOG_PREFIX} "${key}": uploaded → ${info.url}`);
        return { type: "url", value: info.url };
      } catch (e) {
        console.warn(`${LOG_PREFIX} "${key}": upload failed, falling back to inline json:`, e);
      }
    } else {
      console.log(`${LOG_PREFIX} "${key}": json ${formatSize(jsonString.length)} → inline json`);
    }
...
export async function convertMetaframeInputs(
  ...
  console.log(`${LOG_PREFIX} Converting ${entries.length} input(s), threshold=${formatSize(UPLOAD_SIZE_THRESHOLD)}`);
  ...
  console.log(`${LOG_PREFIX} Done: ${uploaded} uploaded to S3, ${inlined} inlined`);
