...
Deno.test("computeMetaframeDefinition returns default when no hashParams", () => {
  const result = computeMetaframeDefinition(undefined);
  assertEquals(result, DEFAULT_METAFRAME_DEFINITION);
});

Deno.test("computeMetaframeDefinition returns default when hashParams has no definition", () => {
  const hashParams = "?js=Y29uc29sZS5sb2coImhpIik=";
  const result = computeMetaframeDefinition(hashParams);
  assertEquals(result, DEFAULT_METAFRAME_DEFINITION);
});

Deno.test("computeMetaframeDefinition merges definition inputs and outputs", () => {
  const definition = {
...
Deno.test("computeMetaframeDefinition includes default hashParams when definition has no hashParams", () => {
  const definition = {
  ...
  const resultKeys = Object.keys(result.hashParams as Record<string, unknown>);
...
Deno.test("computeMetaframeDefinition adds custom hashParams from definition (object-shaped)", () => {
  const definition = {
    hashParams: {
      myCustomParam: {
        type: "string",
      ...
      anotherParam: { type: "json", description: "Another", label: "Another" },
    },
  };
  const hashParams = `?definition=${blobToBase64String(definition)}`;
  const result = computeMetaframeDefinition(hashParams);
  ...
  assertEquals(
    resultHP["js"],
    (DEFAULT_METAFRAME_DEFINITION.hashParams as Record<string, unknown>)["js"],
  );
  assertEquals(
    resultHP["inputs"],
...
Deno.test("computeMetaframeDefinition adds custom hashParams from definition (array-shaped)", () => {
  const definition = {
...
Deno.test("computeMetaframeDefinition overrides metadata from definition", () => {
  const definition = {
...
Deno.test("computeMetaframeDefinition preserves default allow when definition omits it", () => {
  const definition = {
    inputs: { x: { type: "json" } },
  };
  const hashParams = `?definition=${blobToBase64String(definition)}`;
...
Deno.test("getAllowedHashParams includes user-whitelisted params (object-shaped)", () => {
...
Deno.test("getAllowedHashParams includes user-whitelisted params (array-shaped)", () => {
  const allowed = getAllowedHashParams({
    metadata: { name: "x" },
