...
function shouldCache(url, response = null) {
  ...
  if (url.includes('metaframe.json') && (url.includes('localhost') || url.includes('127.0.0.1'))) {
    return false;
  }
  ...
  try {
    ...
    if (response.ok && response.status >= 200 && response.status < 300) {
      ...
      if (shouldCache(finalUrl, response)) {
        ...
        log('🔄 Cached:', request.url, `→ ${finalUrl}`, `(${response.headers.get('content-type') || 'unknown type'})`);
...
self.addEventListener('fetch', event => {
  ...
  if (!config.staleWhileRevalidateEnabled) {
    if (config.enableLogging) {
      log('⚪ Stale-while-revalidate disabled, passing through:', event.request.url);
    }
    return;
  }
  ...
  event.respondWith(
    (async () => {
      ...
      if (cachedResponse) {
        ...
        if (isStale) {
          log('📡 Serving stale content, revalidating in background:', event.request.url);
          ...
          fetchAndCache(event.request).catch(error => {
            log('❌ Background revalidation failed:', error, 'for', event.request.url);
          });
